...
func groupFieldsToRequests(fields []field, tags map[string]string, maxBatchSize uint16) []request {
	...
	requests = append(requests, current)
	return removeEmptyRequests(requests)
}

func removeEmptyRequests(requests []request) []request {
	noEmptyRequests := make([]request, 0, len(requests))
	for _, r := range requests {
		if len(r.fields) != 0 {
			noEmptyRequests = append(noEmptyRequests, r)
		}
	}
	return noEmptyRequests
}
